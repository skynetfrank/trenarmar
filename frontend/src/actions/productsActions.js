import {
  ...
  PRODUCT_LIST_REQUEST, PRODUCT_LIST_SUCCESS, PRODUCT_UPDATE_FAIL, PRODUCT_UPDATE_REQUEST, PRODUCT_UPDATE_SUCCESS
} from "../constants/productsConstants"
import Axios from 'axios';

export const listProducts = ({ pageNumber = '', nombre = '' }) => async (dispatch) => {
  dispatch({ type: PRODUCT_LIST_REQUEST });
  try {
    const { data } = await Axios.get(`/api/productos?pageNumber=${pageNumber}&nombre=${nombre}`);
    //const { cfg } = await Axios.get('/api/appconfig/seed');
...
export const createProduct = () => async (dispatch, getState) => {
  ...
  try {
    const { data } = await Axios.post('/api/productos', {},
      { headers: { Authorization: `Bearer ${userInfo.token}` }, }
    );
    console.log("ProductActions: createProduct =>>: data recibida sel servidor", data);
    dispatch({
      type: PRODUCT_CREATE_SUCCESS, payload: data.producto,
    });
...
export const updateProduct = (producto) => async (dispatch, getState) => {
  ...
  try {
    ...
    console.log("ProductActions: UPDATEPRODUCT =>>: data recibida", data);
  } catch (error) {
    const message =
      error.response && error.response.data.message
        ? error.response.data.message
